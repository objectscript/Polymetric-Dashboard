(function() {
  ...
  core.controller('widgetCtrl', ['$scope', '$element', '$compile', '$timeout', 'WidgetProvider', 'dashboard', function($scope, $element, $compile, $timeout, WidgetProvider, dashboard) {
    ...
    function drawWidget() {
      ...
      var holder = angular.element($element.find('span'));
    ...
    var updateDragListener;
    // on drag, the widgets should be saved so their positions are kept
    $('.grid-stack').on('dragstop', function(event, ui) {
      $timeout.cancel(updateDragListener);

      updateDragListener = $timeout(function() {
        updateWidgetPos();
      }, 250);
    });
